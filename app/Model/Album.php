...
class Album extends AppModel{
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/album/' . $fileName;
		$path_th = WWW_ROOT . 'img/album/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 273, 192, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/album/' . $name)){
			$name = $this->genNameFile($ext);
		}
		return $name;
	}
	public function resizeImg($target, $dest, $wmax = 553, $hmax = 415, $ext){
		/*
